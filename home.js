...
//CODE HERE
const canWeDeliver = zipCode => deliveryAreaZipCodes.includes(zipCode) ? `Hooray! ${zipCode} is in our delivery zone!` : `Sorry, we can't deliver the ${zipCode} area`;
console. log(canWeDeliver(85205), canWeDeliver(84606));
...
// CODE HERE
const canWeDeliverTwo = zipCode => // canWeDeliverTwo is a function in arrow notation that takes in one perameter, zipCode
    deliveryAreaZipCodes.reduce( // then reduces the deliveryAreaZipCodes array to a single boolean
    (found, check)=> found || check === zipCode, false) // by returning the result of the or (||) operation on the 'found' perameter (starting false) and the conditional that check === zipCode
    ? `Hooray! ${zipCode} is in our delivery zone!` : `Sorry, we can't deliver the ${zipCode} area`; // a trinary statement then returns the appropriate message.
    console. log(canWeDeliverTwo(85205), canWeDeliverTwo(84606));
